...
async function validarUserCode() {
  ...
  // Si no hay user_code, permitir continuar (modo legacy sin validación)
  if (!userCodeParam) {
    console.log('No hay user_code en la URL, continuando sin validación');
    return true;
  }
  
  // Verificar que el webhook esté configurado
  if (!CONFIG || !CONFIG.n8n || !CONFIG.n8n.webhookValidateUserCode) {
    console.warn('Webhook de validación no configurado, continuando sin validación');
    return true; // Permitir continuar si no está configurado
  }
  ...
  try {
    ...
    console.log('Respuesta de validación:', responseData);
    ...
    console.log('✅ Validación exitosa:', usuario);
    return true;
    
  } catch (error) {
    console.error('❌ Error en validación:', error);
    mostrarErrorValidacion('El código del conector no es válido o no existe en la base de datos.');
    return false;
  }
}
  ...
  if (sectionContainer) {
    sectionContainer.innerHTML = `
      <div class="bg-red-50 border-2 border-red-200 rounded-2xl p-8 text-center">
        ...
        <h3 class="text-2xl font-bold text-red-800 mb-4">URL No Válida</h3>
    ...
    alert(mensaje + '\n\nPor favor, verifica que estés usando el enlace correcto.\n\n👉 Puedes contactar al conector que te facilitó el enlace o escribirnos directamente a nuestra empresa por WhatsApp:\n\n📲 +34 685 55 53 62');
...
function obtenerAsesorDesdeURL() {
  ...
  // Si no, usar los parámetros de la URL
  if (!usuarioValidado) {
